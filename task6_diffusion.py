...
# ============================================================================
# PART 1: NETWORK DATA (Imported from Tasks 1-2)
# ============================================================================
...
class DiffusionSimulator:
    ...
    def compare_methods(self, t_compare=2.0, dt=0.01):
        ...
        --------
        dict
            Contains analytical solution, numerical solution, and error
        """
        ...
        _, trajectory = self.euler_simulation(
            dt=dt,
            total_time=t_compare + dt
        )
...
def plot_diffusion_over_time(
    ...
    plt.suptitle(
        f'Diffusion from '
        f'{simulator.node_names[simulator.source_node]} '
...
def plot_comparison(
    ...
    results = simulator.compare_methods(
        t_compare,
        dt
    )
    ...
    textstr = (
        f'Max Error: {results["max_error"]:.6f}\n'
    ...
    print(
        ...
        "methods match perfectly!"
    )
    print("=" * 50 + "\n")


def plot_3d_diffusion(
    simulator,
    t=5.0,
    node_positions=None,
    figsize=(12, 8)
...
def main():
    ...
    print("=" * 60)
    print(
        "TASK 6: DIFFUSION (HEAT EQUATION) "
        "ON GHANA ROAD NETWORK"
    )
    print("=" * 60)
    print()
    ...
    print(
        "1. Generating diffusion over time plot..."
    )

    plot_diffusion_over_time(
        simulator
    )

    # 2. Compare analytical and numerical methods
    print(
        "2. Comparing analytical and numerical methods..."
    )
    ...
    try:
        ...
        print(
            "Animation saved. To view, use "
            "Jupyter notebook or save as GIF."
        )
        ...
        try:
            ...
            print(
                ...
                "Install pillow for GIF support."
            )
...
def validate_with_networkx():
    ...
    print("\n" + "=" * 60)
    print(
        "VALIDATION: Comparing with NetworkX (Task 7)"
    )
    print("=" * 60)
    ...
    try:
        ...
        print(
            f"Maximum difference between our L "
            f"and NetworkX L: {max_diff:.6f}"
        )

        print(
            f"Matrices match: "
            f"{np.allclose(our_L, nx_L)}"
        )
        ...
        if n_components == 1:
            ...
            print(
                f"Number of zero eigenvalues: "
                f"{np.sum(np.abs(eigenvals) < 1e-10)}"
            )
